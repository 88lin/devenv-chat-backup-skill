...
do_backup() {
    ...
    local push_rc; timeout "$GIT_TIMEOUT" git push origin HEAD:main 2>/dev/null && push_rc=0 || timeout "$GIT_TIMEOUT" git push origin HEAD:master 2>/dev/null && push_rc=0 || push_rc=1
